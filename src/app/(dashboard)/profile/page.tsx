...
import { Pencil, Lock, Key, AtSign, User, CheckCircle, Save, X, Loader2 } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert('Error al actualizar el protocolo de identidad.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <section className="lg:col-span-7 space-y-10">
          <div className="space-y-5">
            <div className="flex justify-between items-center px-1">
              ...
              {!isEditing ? (
                ...
                <div className="flex items-center gap-4">
                   ...
                   >
                     {isLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Save className="w-3 h-3" />} Guardar
                   </button>
                </div>
              )}
            </div>
            ...
            <div className="bg-[#0a0f16]/40 border border-white/10 rounded-[2.5rem] p-10 space-y-8 backdrop-blur-xl">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2">
                  <label className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] ml-1">ID Corporativo</label>
                  <div className="bg-white/5 p-5 rounded-2xl border border-white/5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2 opacity-60">
                  <label className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] ml-1">Nodo de Contacto (Seguro)</label>
          ...
          <div className="bg-nexus-blue/5 border border-nexus-blue-light/10 rounded-[2rem] p-8 flex items-center gap-6">
             ...
             <div>
                <p className="text-[10px] font-black text-nexus-blue-light uppercase tracking-widest">Protocolo de Cifrado</p>
                <p className="text-xs text-white/60 font-medium">Tu identidad está protegida bajo estándares de cifrado militar y tecnología Ledger.</p>
             </div>
          </div>
        </section>
      </div>
