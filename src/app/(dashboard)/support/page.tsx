...
function CreateTicketModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = () => {
    ...
    mutate(
      ...
      {
        onSuccess: () => {
          addNotification({ type: 'success', title: t('ticketCreated'), message: t('ticketCreatedMsg') });
          onClose();
        },
        onError: (err) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-[#0a0f16] border border-white/10 rounded-3xl shadow-2xl overflow-hidden">
        <div className="px-8 py-6 border-b border-white/5 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-black text-white uppercase tracking-tighter">{t('newTicketTitle')}</h2>
            <p className="text-[10px] text-white/30 font-black uppercase tracking-widest mt-0.5">{t('responseTime')}</p>
          </div>
        ...
        <div className="p-8 space-y-5">
          ...
          >
            {isPending ? <><Loader2 className="w-4 h-4 animate-spin" /> {t('sending')}</> : <><Send className="w-4 h-4" /> {t('newTicket')}</>}
          </button>
        </div>
      </div>
    </div>
...
function TicketThread({ ticket, onBack }: { ticket: SupportTicket; onBack: () => void }) {
  ...
  const STATUS_CONFIG = {
    open:        { label: t('open'),       className: 'bg-amber-500/10 border-amber-500/30 text-amber-400',        icon: AlertCircle },
    in_progress: { label: t('inProgress'), className: 'bg-nexus-blue/10 border-nexus-blue-light/30 text-nexus-blue-light', icon: Clock },
    closed:      { label: t('closed'),     className: 'bg-white/5 border-white/10 text-white/30',                  icon: CheckCircle },
  } as const;
  ...
  const handleReply = () => {
    ...
    mutate(replyText.trim(), {
      onSuccess: () => {
        setReplyText('');
        addNotification({ type: 'success', title: t('replySent'), message: t('replySentMsg') });
      },
      onError: (err) => {
      ...
      <div className="bg-[#0a0f16]/40 border border-white/10 rounded-3xl overflow-hidden backdrop-blur-xl">
        ...
        <div className="p-6 space-y-4 min-h-[300px]">
          {isLoading ? (
            ...
            <p className="text-center text-[10px] text-white/20 font-black uppercase tracking-widest py-16">{t('noMessages')}</p>
          ) : (
            messages.map(msg => {
                ...
                <div key={msg.id} className={`flex ${isAdmin ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-[75%] rounded-2xl px-5 py-4 space-y-1.5 ${
                    ...
                    <p className={`text-[9px] font-black uppercase tracking-widest ${isAdmin ? 'text-nexus-blue-light' : 'text-white/30'}`}>
                      {isAdmin ? t('nexuSupport') : t('you')} · {new Date(msg.created_at).toLocaleDateString('es', { day: '2-digit', month: 'short' })} {new Date(msg.created_at).toLocaleTimeString('es', { hour: '2-digit', minute: '2-digit' })}
                    </p>
        ...
        {isClosed && (
          <div className="px-6 pb-6">
            <div className="flex items-center gap-3 px-5 py-3 bg-white/[0.03] border border-white/5 rounded-2xl">
              ...
              <p className="text-[10px] font-black text-white/20 uppercase tracking-widest">{t('closedMessage')}</p>
            </div>
          </div>
        )}
      </div>
  ...
  if (tickets.length === 0) {
    return (
      <div className="bg-[#0a0f16]/40 border border-white/10 rounded-3xl flex flex-col items-center justify-center py-24 gap-4 backdrop-blur-xl">
        ...
        <p className="text-[10px] font-black text-white/20 uppercase tracking-widest">{t('noTickets')}</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      {meta && meta.last_page > 1 && (
        <div className="flex items-center justify-center gap-3">
          <button disabled={page === 1} onClick={() => setPage(p => p - 1)} className="px-4 py-2 text-[10px] font-black uppercase tracking-widest text-white/40 disabled:opacity-20 hover:text-nexus-blue-light transition-colors">{tCommon('previous')}</button>
          <span className="text-[10px] font-black text-nexus-blue-light">{page} / {meta.last_page}</span>
          <button disabled={page === meta.last_page} onClick={() => setPage(p => p + 1)} className="px-4 py-2 text-[10px] font-black uppercase tracking-widest text-white/40 disabled:opacity-20 hover:text-nexus-blue-light transition-colors">{tCommon('next')}</button>
        </div>
      )}
    </div>
  );
