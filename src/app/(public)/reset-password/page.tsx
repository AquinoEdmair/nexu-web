  ...
  return (
    <div className="flex-grow px-6 pt-12 pb-24 flex flex-col items-center max-w-md mx-auto w-full">
      ...
      <div className="text-center mb-10">
        <h2 className="text-3xl font-black text-white tracking-tighter mb-3 uppercase">{t('title')}</h2>
        <p className="text-nexus-text text-sm font-medium leading-relaxed">{t('subtitle')}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="w-full space-y-6">
        <div className="space-y-2 group">
          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-nexus-text/60 ml-1 group-focus-within:text-nexus-blue-light transition-colors">{t('newPassword')}</label>
        ...
        <div className="space-y-2 group">
          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-nexus-text/60 ml-1 group-focus-within:text-nexus-blue-light transition-colors">{t('confirmPassword')}</label>
          <div className="relative group bg-white/5 border border-white/5 rounded-xl overflow-hidden transition-all duration-300 focus-within:ring-1 focus-within:ring-nexus-blue-light">
            <input
              className="w-full bg-transparent border-none py-4 px-5 text-white placeholder-white/20 focus:ring-0 focus:outline-none font-medium outline-none"
              placeholder="••••••••••••"
          ...
          {password && passwordConfirmation && password !== passwordConfirmation && (
            <p className="text-xs text-red-400 ml-1">{t('passwordMismatch')}</p>
          )}
        </div>
        ...
        <div className="bg-nexus-blue/5 rounded-2xl p-5 border border-nexus-blue-light/10 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-black tracking-widest uppercase text-nexus-blue-light/80">{t('securityProtocol')}</span>
          </div>
      ...
      <div className="mt-12 w-full rounded-3xl overflow-hidden relative h-32 bg-white/5 flex items-center p-6 border border-white/5">
        ...
        <div className="flex-1 z-10">
          <h4 className="text-xs font-black text-white mb-1 uppercase tracking-tighter">{t('securityNote')}</h4>
          <p className="text-[10px] text-nexus-text/60 font-medium leading-tight">{t('securityDetail')}</p>
        </div>
      </div>
    </div>
  );
